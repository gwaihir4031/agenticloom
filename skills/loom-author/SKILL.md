...
- **Persona names resolve via a cli-aware layered lookup** — claude: `<cwd>/.claude/agents/<name>.md`, then `~/.claude/agents/<name>.md`; copilot: `<cwd>/.github/agents/<name>.agent.md`, then `~/.copilot/agents/<name>.agent.md`. Missing persona files fail at compile, not runtime. claude personas must carry frontmatter `name:` equal to the reference AND a non-empty `description:` — claude registers agents by frontmatter name and refuses description-less files; loom validates this at compile time (copilot: existence check only).
